...
.PHONY: build release debug clean test metal cuda rocm cpu help
...
release:
	@chmod +x build_beast.sh
	@BUILD_TYPE=release ./build_beast.sh

# Auto-detect hardware and build debug
debug:
	@chmod +x build_beast.sh
	@BUILD_TYPE=debug ./build_beast.sh
...
# Force ROCm build (AMD)
rocm:
	@echo "🔧 Manual override: Building with ROCm backend"
...
help:
	...
	@echo "  make rocm     - Force ROCm build (AMD)"
	...
	@echo "  ./build_beast.sh              - Auto-detect and build"
	@echo "  FORCE_METAL=1 ./build_beast.sh  - Force Metal"
	@echo "  FORCE_CUDA=1 ./build_beast.sh   - Force CUDA"
	@echo "  FORCE_CPU=1 ./build_beast.sh    - Force CPU-only"
